...
const typeColor = (type) => {
  switch (type) {
    case "Sale":             return "text-blue-500 font-semibold";
    case "Purchase":         return "text-purple-500 font-semibold";
    case "Sale Return":      return "text-orange-500 font-semibold";
    case "Purchase Return":  return "text-rose-500 font-semibold";
    case "Payment Received": return "text-emerald-500 font-semibold";
    case "Payment Paid":     return "text-indigo-500 font-semibold";
    case "Expense":          return "text-red-500 font-semibold";
...
export default function ActivityList() {
  ...
  const renderBody = () => {
    if (loading) {
      return (
        <TableRow>
          <TableCell colSpan={5} className="text-center py-6 text-zinc-400">Loading…</TableCell>
        </TableRow>
      );
    }
    if (error) {
      return (
        <TableRow>
          <TableCell colSpan={5} className="text-center py-4 text-red-400 text-sm">Error: {error}</TableCell>
        </TableRow>
      );
    }
    if (activities.length === 0) {
      return (
        <TableRow>
          <TableCell colSpan={5} className="text-center py-6 text-zinc-400">No recent activity</TableCell>
        </TableRow>
      );
    }
