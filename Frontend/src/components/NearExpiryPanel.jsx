  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Thresholds: {thresholds.critical}/{thresholds.warning}/{thresholds.watch} days</span>
      </div>
      ...
      {!allClear && (
        <div className="grid grid-cols-3 gap-3">
          {LEVELS.map(({ key, label, description, icon: Icon, bg, badge, iconColor, numColor, dot }) => {
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-semibold ${hasAlert ? numColor : "text-gray-400"}`}>
                      {label}
                    </span>
                    {hasAlert && (
                      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${badge}`}>
                  ...
                  <p className={`text-[11px] mt-0.5 ${hasAlert ? "text-gray-500" : "text-gray-300"}`}>
                    {description}
                  </p>
                </div>
              </div>
