...
export default function Manufacturers() {
  ...
  const validate = () => {
    ...
    if (
      form.contact_number &&
      !/^\+?(?:\d[ -]?|\(\d+\)[ -]?){7,20}$/.test(form.contact_number.trim())
    )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        showToast(
          editingId ? 'Manufacturer updated successfully.' : 'Manufacturer added successfully.',
          'success'
        );
        setShowForm(false);
        setEditingId(null);
  ...
  const handleDeactivate = async (id, name) => {
    if (
      !window.confirm(
        `Deactivate "${name}"? Products linked to this manufacturer will be affected.`
      )
    )
      return;
    try {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Manage pharmaceutical manufacturers and DRAP licences
          </p>
        </div>
        <button
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              />
              {fieldErrors.drap_mfg_licence && (
                <p className="text-red-500 text-xs mt-1">{fieldErrors.drap_mfg_licence}</p>
              )}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              {fieldErrors.contact_number && (
                <p className="text-red-500 text-xs mt-1">{fieldErrors.contact_number}</p>
              )}
            </div>
            <div>
              ...
              <input
                ...
                placeholder="e.g. info@gsk.com.pk"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                {['Name', 'DRAP Licence', 'Country', 'Contact', 'Email', 'Status', 'Action'].map(
                  (h) => (
                    <th
                      key={h}
                    ...
                    <p className="text-sm">
                      {searchQuery
                        ...
                        : activeTab === 'deactivated'
                          ? 'No deactivated manufacturers.'
                          : 'No manufacturers yet. Add your first manufacturer above.'}
                    </p>
                  </td>
                </tr>
              ) : (
                  ...
                  .map((mfg) => (
                    <tr key={mfg.manufacturer_id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-gray-600">
                        {mfg.drap_mfg_licence || <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-600">{mfg.country}</td>
                      <td className="px-4 py-3 text-gray-600">
                        {mfg.contact_number || <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3 text-gray-600">
                        {mfg.email || <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      ${mfg.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${mfg.is_active ? 'bg-green-500' : 'bg-gray-400'}`}
                          />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {mfg.is_active ? (
                            <button
                              onClick={() => handleDeactivate(mfg.manufacturer_id, mfg.name)}
                            ...
                            <button
                              onClick={() => handleReactivate(mfg.manufacturer_id, mfg.name)}
