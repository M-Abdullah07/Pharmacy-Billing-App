    ...
    <div className="p-6 h-full w-full max-w-7xl mx-auto flex flex-col bg-gray-50/50">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage goods returned to suppliers and credit notes.</p>
        </div>
        <button
      ...
      {isFormOpen ? (
        <div className="bg-white rounded-xl shadow-sm border p-6 flex flex-col h-full overflow-hidden">
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Select Purchase Invoice</label>
            ...
            {selectedInvoice && (
              <div className="flex flex-col justify-end">
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex justify-between items-center">
                  <div>
                    <p className="text-xs text-blue-600 font-semibold uppercase tracking-wider">Supplier</p>
                    <p className="text-sm font-medium text-blue-900">{selectedInvoice.supplier_name}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-blue-600 font-semibold uppercase tracking-wider">Invoice Total</p>
                    <p className="text-sm font-medium text-blue-900">{formatCurrency(selectedInvoice.net_payable)}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          {selectedInvoice && (
            <div className="flex flex-1 gap-6 min-h-0 overflow-hidden">
              ...
              <div className="flex-1 flex flex-col border rounded-lg overflow-hidden bg-gray-50">
                <div className="p-3 bg-gray-100 border-b font-medium text-sm text-gray-700">Available Items in Invoice</div>
                <div className="overflow-y-auto flex-1 p-2 space-y-2">
                  {invoiceItems.map(item => (
                    <div key={item.item_id} className="bg-white p-3 rounded border shadow-sm flex items-center justify-between">
                      <div>
                        ...
                        {returnItems.map(item => (
                          <tr key={item.item_id}>
                            <td className="py-3">
                              ...
                              <div className="text-xs text-gray-500">{item.batch_number} (Max: {item.quantity})</div>
                            </td>
                            <td className="py-3">
                              <div className="font-medium text-gray-900">{formatCurrency(item.net_unit_cost)}</div>
                              {item.discount_pct > 0 && <div className="text-[10px] text-gray-500">(-{item.discount_pct}%)</div>}
                            </td>
                            <td className="py-3">
                              <input
                                type="number"
                                ...
                                onChange={(e) => handleQuantityChange(item.item_id, e.target.value, item.quantity, item.net_unit_cost)}
                            ...
                            <td className="py-3 text-right">
                              <button onClick={() => handleRemoveReturnItem(item.item_id)} className="text-red-500 hover:text-red-700 p-1">
                                <Trash2 size={16} />
                              </button>
                            </td>
                          </tr>
            ...
            <table className="w-full text-left text-sm text-gray-600">
              ...
              <tbody className="divide-y">
                {loading ? (
                  ...
                  <tr><td colSpan="6" className="text-center py-8 text-gray-400">No return records found.</td></tr>
                ) : (
                  returns.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(ret => (
                    <tr key={ret.return_id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">{formatDate(ret.return_date)}</td>
                      <td className="px-6 py-4 font-medium text-gray-900">{ret.purchase_invoice_number}</td>
                      <td className="px-6 py-4">{ret.supplier_name}</td>
                      <td className="px-6 py-4 capitalize">{ret.reason.replace('_', ' ')}</td>
                      <td className="px-6 py-4 text-right font-semibold text-gray-900">{formatCurrency(ret.total_credit)}</td>
