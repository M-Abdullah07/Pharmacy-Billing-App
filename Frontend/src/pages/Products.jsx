...
const FORM_OPTIONS   = ['Tablet', 'Capsule', 'Syrup', 'Injection', 'Cream', 'Drops', 'Inhaler', 'Powder', 'Gel', 'Patch'];

const EMPTY_FORM = {
  name: '', manufacturer_id: '', category_id: '', form: '',
  uom: 'Strip', quantity_in_uom: 1, hsn_code: '', gst_rate: 0,
  drug_schedule: 'OTC', generic_formula: '', default_sale_rate: '',
  default_purchase_rate: '', shelf_no: '',
};
...
const stockStatusBadge = (status) => ({
  ...
  expiry_warning:  { label: 'Expiry Warning',   cls: 'bg-amber-100 text-amber-700',  dot: 'bg-amber-500' },
  expiry_watch:    { label: 'Expiry Watch',     cls: 'bg-yellow-100 text-yellow-700',dot: 'bg-yellow-500' },
      ...
      <div className="fixed right-0 top-0 h-full w-[420px] bg-white border-l border-gray-200 shadow-xl z-50 flex flex-col">
        ...
        <div className="flex items-start justify-between p-5 border-b border-gray-100">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded-full ${status.cls}`}>
        ...
        <div className="grid grid-cols-3 gap-3 p-5 border-b border-gray-100">
          <div className="bg-gray-50 rounded-lg p-3 text-center">
            ...
            <p className={`text-lg font-bold ${product.total_quantity === 0 ? 'text-red-500' : 'text-gray-900'}`}>
        ...
        <div className="px-5 py-4 border-b border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-y-2 text-xs">
            {[
              ...
              ['Sale Rate',   product.default_sale_rate ? `Rs ${Number(product.default_sale_rate).toLocaleString()}` : '—'],
            ].map(([label, value]) => (
              <div key={label}>
            ...
            <div className="px-5 pb-5 space-y-2">
              {batches.map(b => {
                ...
                const expiryStatusCls =
                  days <= 30 ? 'border-red-200 bg-red-50/50' :
                  days <= 60 ? 'border-amber-200 bg-amber-50/50' :
                  days <= 90 ? 'border-yellow-200 bg-yellow-50/50' :
                               'border-gray-100 bg-white';
                return (
                  <div key={b.batch_id}
                    className={`border rounded-lg p-3 ${expiryStatusCls}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold text-gray-800 font-mono">{b.batch_number}</span>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700 font-medium">Rs {Number(b.mrp).toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-gray-400">Available</p>
                        <p className={`font-semibold ${b.quantity_available === 0 ? 'text-red-500' : 'text-gray-900'}`}>
                          {b.quantity_available === 0 ? 'Out' : b.quantity_available.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
...
export default function Products() {
  ...
  const handleOpenForm = () => {
    if (manufacturers.length === 0) { setNoMfgWarning(true); return; }
  ...
  const handleEdit = (product) => {
    setForm({
      name: product.name || '', manufacturer_id: product.manufacturer_id || '',
      category_id: product.category_id || '', form: product.form || '',
      uom: product.uom || 'Strip', quantity_in_uom: product.quantity_in_uom || 1,
  ...
  const handleDeactivate = async (id, name) => {
    ...
    try {
      ...
      if (result.success) { showToast(`"${name}" deactivated.`, 'success'); await loadAll(); }
  ...
  const handleReactivate = async (id, name) => {
    ...
    try {
      ...
      if (result.success) { showToast(`"${name}" reactivated.`, 'success'); await loadAll(); }
      else showToast(result.error || 'Failed.', 'error');
    } catch (err) { showToast('Service unavailable.', 'error'); }
  };
  ...
  const uniqueManufacturers = [...new Set(stockSummary.map(s => s.manufacturer_name).filter(Boolean))].sort();
    ...
    <div className="w-full h-full flex flex-col gap-4 p-1">
          ...
          {noMfgWarning && (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
              ...
              <div>
                ...
                <p className="mt-0.5 text-amber-700">Please go to <span className="font-semibold">Manufacturers</span> and add at least one first.</p>
              </div>
              <button onClick={() => setNoMfgWarning(false)} className="ml-auto shrink-0 opacity-60 hover:opacity-100"><X size={16} /></button>
            </div>
          )}
          ...
          {showForm && (
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h2 className="text-base font-semibold text-gray-900">{editingId ? 'Edit Product' : 'Add New Medicine'}</h2>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <input type="text" value={form.name} onChange={e => field('name', e.target.value)}
                    placeholder="e.g. Amoxicillin 500mg" className={inputCls('name')} />
                  {fieldErrors.name && <p className="text-red-500 text-xs mt-1">{fieldErrors.name}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Manufacturer <span className="text-red-500">*</span></label>
                  <select value={form.manufacturer_id} onChange={e => field('manufacturer_id', e.target.value)} className={selectCls('manufacturer_id')}>
                    <option value="">Select Manufacturer</option>
                    {manufacturers.map(m => <option key={m.manufacturer_id} value={m.manufacturer_id}>{m.name}</option>)}
                  </select>
                  {fieldErrors.manufacturer_id && <p className="text-red-500 text-xs mt-1">{fieldErrors.manufacturer_id}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Category <span className="text-red-500">*</span></label>
                  <select value={form.category_id} onChange={e => field('category_id', e.target.value)} className={selectCls('category_id')}>
                    <option value="">Select Category</option>
                    {categories.map(c => <option key={c.category_id} value={c.category_id}>{c.name}</option>)}
                  </select>
                  {fieldErrors.category_id && <p className="text-red-500 text-xs mt-1">{fieldErrors.category_id}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                <div>
                  ...
                  <select value={form.form} onChange={e => field('form', e.target.value)} className={selectCls('form')}>
                    <option value="">Select Form</option>
                    {FORM_OPTIONS.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Pack Size (UOM)</label>
                  <select value={form.uom} onChange={e => field('uom', e.target.value)} className={selectCls('uom')}>
                ...
                <div>
                  ...
                  <input type="text" value={form.hsn_code} onChange={e => field('hsn_code', e.target.value)}
                    placeholder="e.g. 3004" className={inputCls('hsn_code')} />
                  {fieldErrors.hsn_code && <p className="text-red-500 text-xs mt-1">{fieldErrors.hsn_code}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-4 mb-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">FBR GST Rate <span className="text-red-500">*</span></label>
                  <select value={form.gst_rate} onChange={e => field('gst_rate', Number(e.target.value))} className={selectCls('gst_rate')}>
                    {GST_SLABS.map(g => <option key={g} value={g}>{g}%</option>)}
                  </select>
                  {fieldErrors.gst_rate && <p className="text-red-500 text-xs mt-1">{fieldErrors.gst_rate}</p>}
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">DRAP Schedule <span className="text-red-500">*</span></label>
                  <select value={form.drug_schedule} onChange={e => field('drug_schedule', e.target.value)} className={selectCls('drug_schedule')}>
                    {DRUG_SCHEDULES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  {fieldErrors.drug_schedule && <p className="text-red-500 text-xs mt-1">{fieldErrors.drug_schedule}</p>}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Generic Formula</label>
                  <input type="text" value={form.generic_formula} onChange={e => field('generic_formula', e.target.value)}
                    placeholder="e.g. Amoxicillin 500mg" className={inputCls('generic_formula')} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Shelf / Rack No</label>
                  <input type="text" value={form.shelf_no} onChange={e => field('shelf_no', e.target.value)}
                    placeholder="e.g. A-12" className={inputCls('shelf_no')} />
                </div>
              </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
              {[
                ...
                <button key={tab.key} onClick={() => setProdTab(tab.key)}
            ...
            <div className="relative">
              ...
              <input type="text" value={prodSearchQuery} onChange={e => setProdSearchQuery(e.target.value)}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {['Medicine Name', 'Manufacturer', 'Category', 'Schedule', 'GST', 'Status', 'Action'].map(h => (
                  ...
                  {filteredProducts.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="text-center py-12 text-gray-400">
                        ...
                        <p className="text-sm">{prodSearchQuery ? 'No products match your search.'
                          : prodTab === 'discontinued' ? 'No discontinued products.'
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {product.is_active ? (
                            ...
                            <button onClick={() => handleReactivate(product.product_id, product.name)}
      ...
      {pageTab === 'stock' && (
        <>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { label: 'Total Products',  value: stockKpis.total,      sub: 'in master',         cls: 'border-gray-200' },
              { label: 'Out of Stock',    value: stockKpis.outOfStock,  sub: 'need restocking',   cls: 'border-red-200 bg-red-50/50' },
              { label: 'Expiry Critical', value: stockKpis.critical,    sub: '≤ 30 days',          cls: 'border-red-200 bg-red-50/30' },
              { label: 'Total Stock Value', value: `Rs ${stockKpis.totalValue.toLocaleString()}`, sub: 'at MRP', cls: 'border-blue-200 bg-blue-50/30' },
            ].map(kpi => (
          ...
          <div className="flex items-center gap-3">
            ...
            <select value={stockMfgFilter} onChange={e => setStockMfgFilter(e.target.value)}
              className="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer">
              <option value="">All Manufacturers</option>
              {uniqueManufacturers.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
            {(stockSearch || stockStatusFilter !== 'all' || stockMfgFilter) && (
              <button onClick={() => { setStockSearch(''); setStockStatusFilter('all'); setStockMfgFilter(''); }}
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            {loading ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {['Product', 'Manufacturer', 'Category', 'Total Qty', 'Nearest Expiry', 'Stock Value', 'Status', ''].map(h => (
                      <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filteredStock.length === 0 ? (
                    <tr>
                      <td colSpan="8" className="text-center py-12 text-gray-400">
                        ...
                        <p className="text-sm">{stockSearch || stockStatusFilter !== 'all' || stockMfgFilter
                    ...
                    return (
                      <tr
                        ...
                        <td className="px-4 py-3">
                          ...
                          {s.generic_formula && <div className="text-xs text-gray-400 mt-0.5">{s.generic_formula}</div>}
                        </td>
                        <td className="px-4 py-3 text-gray-600 text-xs">{s.manufacturer_name || '—'}</td>
                        <td className="px-4 py-3">
                          {s.category_name
                            ? <span className="inline-flex px-2 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 rounded-full">{s.category_name}</span>
                            : <span className="text-gray-300">—</span>}
                        </td>
                        <td className="px-4 py-3">
                          <span className={`font-semibold ${isOutOfStock ? 'text-red-500' : 'text-gray-900'}`}>
                          ...
                          {s.nearest_expiry ? (
                            <div>
                              <p className="text-xs text-gray-600">
                                {new Date(s.nearest_expiry).toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' })}
                              </p>
                              <p className={`text-xs font-medium ${expiry.cls}`}>{nearest > 0 ? `${nearest}d left` : 'Expired'}</p>
                            </div>
      ...
      {selectedProduct && pageTab === 'stock' && (
        <StockSidePanel
          ...
          onClose={() => { setSelectedProduct(null); setSidePanelBatches([]); }}
