...
const { calculateFefoPlan } = require(path.join(__dirname, '..', '..', '..', 'Backend', 'app', 'utils', 'fefoAllocator.js'));
...
ipcMain.handle("add-sale", async (event, data) => {
  ...
  try {
    ...
    // 6. Update aggregations on parent
    await client.query(`
      ...
      SET subtotal = (SELECT COALESCE(SUM(quantity * sale_rate), 0) FROM sale_invoice_items WHERE sale_invoice_id = $1),
          ...
          amount_paid = CASE WHEN $2::boolean THEN 0 ELSE (SELECT COALESCE(SUM(line_total + tax_amount), 0) FROM sale_invoice_items WHERE sale_invoice_id = $1) END
      WHERE sale_invoice_id = $1
    `, [saleInvoiceId, data.isCredit]);
