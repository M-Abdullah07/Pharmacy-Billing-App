...
COMMENT ON TABLE  contact_persons          IS 'Multiple contacts per supplier or customer. entity_type + entity_id is a polymorphic reference.';
...
CREATE TABLE batches (
    ...
    CONSTRAINT chk_batches_qty         CHECK (quantity_available <= quantity_received)
);
...
-- invoice_number is a candidate key (generated by system). ✓
...
COMMENT ON COLUMN payments.party_id     IS 'UUID of customer (direction=received) or supplier (direction=paid). Enforced at application layer.';
...
BEGIN
    ...
    -- Guard: quantity_available must not go negative
    IF (SELECT quantity_available FROM batches WHERE batch_id = NEW.batch_id) < 0 THEN
        RAISE EXCEPTION
            'Insufficient stock for batch_id %. Movement would result in negative quantity.',
            NEW.batch_id;
    END IF;
